...
|----------------------|---------------------------------------------------------------------------------------------------------------------------------|
| Arquitetura          | Toda a árvore `src/main/java/com/jpmns/task/` (estrutura de pacotes)                                                           |
...
| Referência da API    | Todos os controllers em `src/main/java/com/jpmns/task/core/presentation/controller/` e suas interfaces de documentação         |
...
| Qualidade de Código  | `build.gradle.kts` (configuração do Checkstyle), `checkstyle.xml`                                                              |
...
### Referência da API
- Leia **todos** os controllers e seus respectivos `*ControllerDoc` para extrair os endpoints.
- Para cada endpoint, confirme: método HTTP, path completo, se exige autenticação e o que faz.
- Os exemplos de payload (JSON) devem refletir os campos reais dos `record`s de request/response em `presentation/controller/payload/`.
...
- Mencione todos os tipos de teste presentes: unitários, integração, MockMvc.

### Qualidade de Código
- Confirme o comando correto para executar Checkstyle + testes + cobertura em `build.gradle.kts`.
- Mencione a versão do Checkstyle se estiver declarada.
...
2. **Leia `build.gradle.kts`** para coletar versões, dependências, cobertura mínima e comandos de build.
3. **Leia `src/main/resources/application.yaml`** para mapear variáveis de ambiente.
4. **Explore a árvore `src/main/java/`** para verificar a estrutura de pacotes.
...
- [ ] `build.gradle.kts` lido — versões de dependências conferidas
- [ ] `application.yaml` lido — todas as variáveis de ambiente verificadas
- [ ] Estrutura de pacotes explorada — árvore de arquitetura atualizada se necessário
- [ ] Todos os controllers lidos — tabelas de endpoints conferidas
